...
void dijkstra_search
	(const Graph& graph,
	 typename Graph::Location start,
	 vector<typename Graph::Location> goal,
	 unordered_map<typename Graph::Location, typename Graph::Location>& came_from,
	 unordered_map<typename Graph::Location, double>& cost_so_far)
{
	typedef typename Graph::Location Location;
	...
	while (!frontier.empty() && !exit) {
		...
		for(typename vector<typename Graph::Location>::const_iterator iterator = goal.begin(); iterator != goal.end(); ++iterator)
		{
			if (current == (*iterator)) {
				exit = true;
				break;
